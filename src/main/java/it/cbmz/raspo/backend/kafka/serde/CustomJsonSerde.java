package it.cbmz.raspo.backend.kafka.serde;
/*
...
public class CustomJsonSerde<T> implements Serde<T> {
	...
	public CustomJsonSerde(ObjectMapper objectMapper) {
	...
	public CustomJsonSerde(Class<T> targetType, ObjectMapper objectMapper) {
		...
		this.jsonDeserializer = new CustomJsonDeserializer<>(targetType, objectMapper);
	}

	public CustomJsonSerde(JsonSerializer<T> jsonSerializer, JsonDeserializer<T> jsonDeserializer) {
	...
	/**
	 * Configure the TypeMapper to use key types if the CustomJsonSerde is used to serialize keys.
	 * @param isKey Use key type headers if true
	 * @return the CustomJsonSerde
	 * @since 2.1.3
	 */
	public CustomJsonSerde<T> setUseTypeMapperForKey(boolean isKey) {
